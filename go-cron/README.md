...
```bash
go-cron$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
go-cron$ sam logs -n HelloWorldFunction --stack-name go-cron --tail
```
